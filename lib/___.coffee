...
if Meteor.isServer
  ...
  Mongo.Collection::getIndexes = ->
    raw = @rawCollection()
    future = new Future
    raw.indexes (err, indexes) ->
      if err
        future.throw err
...
if Meteor.isClient
  ...
  Template.registerHelper 'config', -> return configs.findOne({_id: 'config'})
  ReactiveVar.prototype.setItem = (field, value) ->
    tmp = this.get()
    tmp[field] = value
    @set(tmp)
...
@___MODE = do -> process?.env?.___MODE
...
@CheckTimer = class
  ...
  log: (name) ->
    now = new Date()
    console.log "#{name}: #{now.getTime() - @lastTime.getTime()}"
    @lastTime = now
    return
...
unless @___ then @___ =
  error: (reason, error, details) -> return throw new Meteor.Error error, reason, details
  getDistance: (gps1, gps2, unit) ->
    lat1 = gps1.latitude
    lon1 = gps1.longitude
    lat2 = gps2.latitude
    lon2 = gps2.longitude
...
#admin / company / center 의 userInfo를 넘기면 하위의 group_ids를 찾아서 [] return
...
###
  excelDownload: (table_id, fileName) ->
#getting values of current time for generating the file name
    ...
    a.download = fileName + postfix + '.xls'
